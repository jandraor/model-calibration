...
print(draw_density(r_noughts, specs_list))
print(output_sp$theoretical_R0)
```

## 2.2 Symmetric matrix

```{r}
filename <- "SEIR_symmetric_2.2.stan"
params_prior <- "  params ~ normal(0, 1000)"
o_SEIR_sym <- write_SEIR_symmmetric(filename, output_gsi$stocks, params_prior)
...
```{r}
posterior_df <- as.data.frame(stan_fit_2.2)
...
samples                 <- rstan::extract(stan_fit_2.2)
param_samples           <- samples$params
colnames(param_samples) <- o_SEIR_sym$params
param_medians           <- apply(param_samples, 2, function(col) median(col))
